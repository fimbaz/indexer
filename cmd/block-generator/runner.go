...
func init() {
	...
	runnerCmd.Flags().BoolVarP(&runnerArgs.Validate, "validate", "", false, "If set database writes will be validated after adding each block and the validator tool will run after test-duration has elapsed to verify data is correct. An extra line in each report indicates validator success or failure.")
